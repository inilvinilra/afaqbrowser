...
fn write_nss_prelude(out: &Path) {
    fs::write(
        out,
        r#"pub mod SECItemType {
...
pub use self::SECItemType::Type as SECItemType;
...
pub use self::_SECStatus::Type as SECStatus;
extern "C" {
...
fn build_bindings(base: &str, bindings: &Bindings, flags: &[String], gecko: bool) {
    ...
    let mut bindgen_flags = flags.to_vec();
    if !bindings.cplusplus {
        let mut filtered = Vec::new();
        let mut skip_next = false;
        for flag in bindgen_flags {
            if skip_next {
                skip_next = false;
                continue;
            }
            ...
            if flag == "-fno-sized-deallocation" || flag == "-fno-aligned-new" {
                continue;
            }
            filtered.push(flag);
        }
        bindgen_flags = filtered;
        bindgen_flags.extend(["-x".into(), "c".into()]);
    }

    builder = builder.clang_args(&bindgen_flags);
