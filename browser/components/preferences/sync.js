...
var gSyncPane = {
  ...
  _init() {
    ...
    if (
      ...
      Services.prefs.getBoolPref("identity.fxaccounts.pairing.enabled", true)
    ) {
      FxAccounts.config
        .promiseConnectDeviceURI(this._getEntryPoint())
        .then(connectURI => {
          connectAnotherDevice.setAttribute("href", connectURI);
        });
    }

    for (let platform of ["android", "ios"]) {
      let baseURL = Services.prefs.getCharPref(`identity.mobilepromo.${platform}`);
      if (!baseURL) {
        continue;
      }
      let url = baseURL + "sync-preferences";
