...
ChromeUtils.defineESModuleGetters(this, {
  AfaqCleanupService: "resource:///modules/AfaqCleanupService.sys.mjs",
  AfaqSecurityLevelService:
...
Preferences.addAll([
  ...
  // Clear Private Data
  { id: "afaq.security.level", type: "string" },
  { id: "afaq.security.lastAppliedLevel", type: "string" },
  { id: "afaq.security.webrtcMode", type: "string" },
  { id: "afaq.security.compatibilityMode", type: "string" },
  { id: "dom.maxHardwareConcurrency", type: "int" },
  { id: "dom.maxtouchpoints.testing.value", type: "int" },
  { id: "dom.w3c_touch_events.enabled", type: "int" },
  ...
  { id: "browser.urlbar.speculativeConnect.enabled", type: "bool" },
  { id: "network.dns.disablePrefetch", type: "bool" },
  { id: "dom.event.clipboardevents.enabled", type: "bool" },
  { id: "pdfjs.enableScripting", type: "bool" },
  { id: "afaq.cleanup.mode", type: "string" },
  { id: "afaq.cleanup.exit.browsingHistoryAndDownloads", type: "bool" },
  { id: "afaq.cleanup.exit.cookiesAndStorage", type: "bool" },
  { id: "afaq.cleanup.exit.cache", type: "bool" },
  { id: "afaq.cleanup.exit.formdata", type: "bool" },
  { id: "afaq.cleanup.exit.siteSettings", type: "bool" },
...
var gPrivacyPane = {
  _pane: null,
  _afaqCleanupSummaryRequestId: 0,
   ...
   */
  init() {
    this._syncAfaqSecurityLevelControls();
    this._syncAfaqCleanupControls();
    ...
    Preferences.get("afaq.cleanup.mode").on(
      "change",
      gPrivacyPane._syncAfaqCleanupControls.bind(gPrivacyPane)
    );
    Preferences.get("afaq.security.level").on(
      "change",
      gPrivacyPane._syncAfaqSecurityLevelControls.bind(gPrivacyPane)
    );
    Preferences.get("afaq.security.lastAppliedLevel").on(
      "change",
      gPrivacyPane._syncAfaqSecurityLevelControls.bind(gPrivacyPane)
    );
    for (let pref of [
      ...
      Preferences.get(pref).on(
        "change",
        gPrivacyPane._syncAfaqAdvancedSecurityControls.bind(gPrivacyPane)
      );
    }
    Preferences.get("afaq.cleanup.exit.cookiesAndStorage").on(
      "change",
      gPrivacyPane._syncAfaqCleanupControls.bind(gPrivacyPane)
    );
    Preferences.get("afaq.cleanup.exit.cache").on(
      "change",
      gPrivacyPane._syncAfaqCleanupControls.bind(gPrivacyPane)
    );
    Preferences.get("browser.privatebrowsing.autostart").on(
    ...
    setEventListener(
      "clearDataExceptions",
      "command",
      gPrivacyPane.showAfaqCleanupExceptions
    );
    setEventListener(
      "afaqSecurityLevel",
      "command",
      gPrivacyPane.onAfaqSecurityLevelCommand
    );
    setEventListener(
      "afaqSecurityDohMode",
      "command",
      gPrivacyPane.onAfaqSecurityDoHCommand
    );
    setEventListener(
      "afaqSecurityWebRTCMode",
      "command",
      gPrivacyPane.onAfaqSecurityWebRTCModeCommand
    );
    setEventListener(
      "afaqSecurityRemoteSafety",
      "command",
      gPrivacyPane.onAfaqSecurityRemoteSafetyCommand
    );
    setEventListener(
      "afaqSecurityRestoreDefaults",
      "command",
      gPrivacyPane.onAfaqSecurityRestoreDefaultsCommand
    );
    setEventListener(
      "afaqSecurityCompatibilityMode",
      "command",
      gPrivacyPane.onAfaqSecurityCompatibilityModeCommand
    );
    for (let id of [
      "afaqSecurityDisableDnsPrefetch",
      "afaqSecurityAllowSpeculativeConnections",
      "afaqSecurityLetterboxing",
      "afaqSecurityDisableWebGL",
      "afaqSecurityAllowClipboardEvents",
      "afaqSecurityAllowPdfScripting",
    ]) {
      setEventListener(id, "command", gPrivacyPane._syncAfaqAdvancedSecurityControls);
    }
    setEventListener("alwaysClear", "command", gPrivacyPane.onAfaqAlwaysClear);
    setEventListener(
  ...
  prefsForKeepingHistory: {
    ...
    "afaq.cleanup.mode": "keep", // Cleanup mode keeps persistent browsing by default
  },
  ...
  updateHistoryModePrefs() {
    ...
    switch (document.getElementById("historyMode").value) {
      case "remember":
        ...
        Preferences.get("afaq.cleanup.mode").value = "keep";
        break;
      case "dontremember":
        if (!pref.value) {
          pref.value = true;
  ...
  _getAfaqCleanupModePreference() {
    return Preferences.get("afaq.cleanup.mode");
  },

  _getAfaqSecurityLevelPreference() {
    return Preferences.get("afaq.security.level");
  },

  _getAfaqValidSecurityLevel(level) {
    if (["standard", "hardened", "maximum"].includes(level)) {
      return level;
    }
    return "hardened";
  },

  _getAfaqSecurityLevelDescriptionL10nId(level) {
    switch (level) {
      case "standard":
        return "afaq-security-level-summary-standard";
      case "maximum":
        return "afaq-security-level-summary-maximum";
      default:
        return "afaq-security-level-summary-hardened";
    }
  },

  async _syncAfaqSecurityLevelControls() {
    let levelControl = document.getElementById("afaqSecurityLevel");
    let levelDescription = document.getElementById(
      "afaqSecurityLevelDescription"
    );
    let levelWarning = document.getElementById("afaqSecurityLevelWarning");
    if (!levelControl || !levelDescription || !levelWarning) {
      return;
    }

    let levelPref = this._getAfaqSecurityLevelPreference();
    let level = this._getAfaqValidSecurityLevel(levelPref.value);
    let appliedLevel = this._getAfaqValidSecurityLevel(
      Preferences.get("afaq.security.lastAppliedLevel").value || level
    );

    levelControl.value = level;
    levelControl.disabled = levelPref.locked;
    document.l10n.setAttributes(
      levelDescription,
      this._getAfaqSecurityLevelDescriptionL10nId(level),
      { appliedLevel }
    );
    document.l10n.setAttributes(
      levelWarning,
      "afaq-security-level-warning-maximum"
    );
    levelWarning.hidden = level != "maximum";
    await document.l10n.translateElements([levelDescription, levelWarning]);
    await this._syncAfaqAdvancedSecurityControls();
  },

  onAfaqSecurityLevelCommand(event) {
    let level = this._getAfaqValidSecurityLevel(event.target.value);
    this._getAfaqSecurityLevelPreference().value = level;
    AfaqSecurityLevelService.applyCurrentLevel();
    void this._syncAfaqSecurityLevelControls();
  },

  onAfaqSecurityRestoreDefaultsCommand() {
    AfaqSecurityLevelService.applyCurrentLevel();
    void this._syncAfaqSecurityLevelControls();
  },

  onAfaqSecurityCompatibilityModeCommand(event) {
    let value = event.target.value == "troubleshoot" ? "troubleshoot" : "balanced";
    Preferences.get("afaq.security.compatibilityMode").value = value;
    AfaqSecurityLevelService.applyCompatibilityMode(value);
    void this._syncAfaqSecurityLevelControls();
  },

  _getAfaqSafeBrowsingPrefs() {
    return [
      Preferences.get("browser.safebrowsing.malware.enabled"),
      Preferences.get("browser.safebrowsing.phishing.enabled"),
      Preferences.get("browser.safebrowsing.downloads.enabled"),
      Preferences.get(
        "browser.safebrowsing.downloads.remote.block_potentially_unwanted"
      ),
      Preferences.get("browser.safebrowsing.downloads.remote.block_uncommon"),
    ];
  },

  _isAfaqRemoteSafetyEnabled() {
    return this._getAfaqSafeBrowsingPrefs().some(pref => pref.value);
  },

  _isAfaqRemoteSafetyLocked() {
    return this._getAfaqSafeBrowsingPrefs().some(pref => pref.locked);
  },

  _getAfaqDoHModeLabel(mode) {
    switch (Number(mode)) {
      case 0:
        return "Off";
      case 2:
        return "Mullvad DNS with fallback";
      default:
        return "Mullvad DNS only";
    }
  },

  _getAfaqOnOffLabel(value) {
    return value ? "on" : "off";
  },

  _getAfaqContentBlockingLabel(value) {
    return value == "strict" ? "strict" : "custom";
  },

  _getAfaqWebRTCModeLabel(value) {
    return value == "disabled" ? "disabled" : "protected";
  },

  _getAfaqCompatibilityModeLabel(value) {
    return value == "troubleshoot" ? "troubleshoot" : "balanced";
  },

  _getAfaqCookieBehaviorLabel(value) {
    return Number(value) == 4 ? "tracker-only" : "partitioned-third-party";
  },

  async _syncAfaqAdvancedSecurityControls() {
    ...
    let advancedSummary = document.getElementById("afaqSecurityAdvancedSummary");
    ...
    if (
      ...
      !diagnosticsSummary ||
      !diagnosticsWebRtcExpectation
    ) {
      return;
    }

    let dohPref = Preferences.get("network.trr.mode");
    let mode = [0, 2, 3].includes(Number(dohPref.value))
      ? Number(dohPref.value)
      : 3;
    dohMode.value = String(mode);
    dohMode.disabled = dohPref.locked;

    let webRtcModePref = Preferences.get("afaq.security.webrtcMode");
    let webRtcValue =
      webRtcModePref.value == "disabled" ? "disabled" : "protected";
    webRtcMode.value = webRtcValue;
    webRtcMode.disabled = webRtcModePref.locked;

    let compatibilityModePref = Preferences.get("afaq.security.compatibilityMode");
    let compatibilityValue =
      compatibilityModePref.value == "troubleshoot"
        ? "troubleshoot"
        : "balanced";
    compatibilityMode.value = compatibilityValue;
    compatibilityMode.disabled = compatibilityModePref.locked;

    remoteSafety.checked = this._isAfaqRemoteSafetyEnabled();
    remoteSafety.disabled = this._isAfaqRemoteSafetyLocked();

    document.l10n.setAttributes(
      networkSummary,
      "afaq-security-network-summary",
      {
        doh: this._getAfaqDoHModeLabel(mode),
        dnsPrefetch: this._getAfaqOnOffLabel(
          Preferences.get("network.dns.disablePrefetch").value
        ),
        speculative: this._getAfaqOnOffLabel(
          Preferences.get("browser.urlbar.speculativeConnect.enabled").value
        ),
        remoteSafety: this._getAfaqOnOffLabel(remoteSafety.checked),
      }
    );
    document.l10n.setAttributes(
      advancedSummary,
      "afaq-security-advanced-summary",
      {
        letterboxing: this._getAfaqOnOffLabel(
          Preferences.get("privacy.resistFingerprinting.letterboxing").value
        ),
        webRtc: this._getAfaqWebRTCModeLabel(webRtcValue),
        webGl: this._getAfaqOnOffLabel(
          !Preferences.get("webgl.disabled").value
        ),
        clipboardEvents: this._getAfaqOnOffLabel(
          Preferences.get("dom.event.clipboardevents.enabled").value
        ),
        pdfScripting: this._getAfaqOnOffLabel(
          Preferences.get("pdfjs.enableScripting").value
        ),
      }
    );
    document.l10n.setAttributes(
      compatibilitySummary,
      "afaq-security-compatibility-summary",
      {
        profile: this._getAfaqCompatibilityModeLabel(compatibilityValue),
        doh: this._getAfaqDoHModeLabel(
          Preferences.get("network.trr.mode").value
        ),
        cookies: this._getAfaqCookieBehaviorLabel(
          Preferences.get("network.cookie.cookieBehavior").value
        ),
        queryStripping: this._getAfaqOnOffLabel(
          Preferences.get("privacy.query_stripping.enabled").value
        ),
        httpsOnly: this._getAfaqOnOffLabel(
          Preferences.get("dom.security.https_only_mode").value
        ),
      }
    );
    document.l10n.setAttributes(
      diagnosticsSummary,
      "afaq-security-diagnostics-summary",
      {
        webRtcPolicy: this._getAfaqWebRTCModeLabel(webRtcValue),
        webRtcApi: this._getAfaqOnOffLabel(
          Preferences.get("media.peerconnection.enabled").value
        ),
        doh: this._getAfaqDoHModeLabel(
          Preferences.get("network.trr.mode").value
        ),
        ...
        contextMenu: this._getAfaqOnOffLabel(
          !Services.prefs.getBoolPref("dom.event.contextmenu.enabled", true)
        ),
        locale:
          Preferences.get("privacy.spoof_english").value == 2
            ? "English"
            : "system",
        fonts:
          Number(Preferences.get("layout.css.font-visibility").value) == 1
            ? "base"
            : "extended",
        canvas:
          Preferences.get("privacy.fingerprintingProtection.overrides").value
            .includes("CanvasImageExtractionPrompt")
            ? "prompted"
            : "randomized",
        hardwareConcurrency: String(
          Preferences.get("dom.maxHardwareConcurrency").value
        ),
        touch:
          Number(Preferences.get("dom.w3c_touch_events.enabled").value) == 0 &&
          Number(Preferences.get("dom.maxtouchpoints.testing.value").value) == 0
            ? "off"
            : "available",
      }
    );
    document.l10n.setAttributes(
      diagnosticsWebRtcExpectation,
      webRtcValue == "disabled"
        ? "afaq-security-diagnostics-webrtc-expectation-disabled"
        : "afaq-security-diagnostics-webrtc-expectation-protected"
    );
  ...
  onAfaqSecurityDoHCommand(event) {
    let value = Number(event.target.value);
    Preferences.get("network.trr.mode").value = [0, 2, 3].includes(value)
      ? value
      : 3;
    void this._syncAfaqAdvancedSecurityControls();
  },

  onAfaqSecurityWebRTCModeCommand(event) {
    let value = event.target.value == "disabled" ? "disabled" : "protected";
    Preferences.get("afaq.security.webrtcMode").value = value;
    AfaqSecurityLevelService.applyWebRTCMode(value);
    void this._syncAfaqAdvancedSecurityControls();
  },

  onAfaqSecurityRemoteSafetyCommand(event) {
    let value = Boolean(event.target.checked);
    for (let pref of this._getAfaqSafeBrowsingPrefs()) {
      if (!pref.locked) {
        pref.value = value;
      }
    }
  ...
  _getAfaqExitCleanupPrefs() {
    return {
      browsingHistoryAndDownloads: Preferences.get(
        "afaq.cleanup.exit.browsingHistoryAndDownloads"
      ),
      cookiesAndStorage: Preferences.get("afaq.cleanup.exit.cookiesAndStorage"),
      cache: Preferences.get("afaq.cleanup.exit.cache"),
      formdata: Preferences.get("afaq.cleanup.exit.formdata"),
      siteSettings: Preferences.get("afaq.cleanup.exit.siteSettings"),
    };
  },

  _isAfaqExitCleanupEnabled() {
    return this._getAfaqCleanupModePreference().value == "exit";
  },

  _hasAfaqSelectedExitCategories() {
    return Object.values(this._getAfaqExitCleanupPrefs()).some(pref => pref.value);
  },

  _ensureAfaqDefaultExitCategories() {
    let prefs = this._getAfaqExitCleanupPrefs();
    if (this._hasAfaqSelectedExitCategories()) {
      return;
    }

    prefs.cookiesAndStorage.value = true;
    prefs.cache.value = true;
  },

  _updateDeleteOnCloseBoxFromPrefs() {
    ...
    deleteOnCloseBox.checked =
      privateBrowsing ||
      (this._isAfaqExitCleanupEnabled() &&
        prefs.cookiesAndStorage.value &&
        prefs.cache.value);
  },

  _syncAfaqCleanupControls() {
    let alwaysClearBox = document.getElementById("alwaysClear");
    let settingsButton = document.getElementById("clearDataSettings");
    let privateBrowsing = Preferences.get(
      "browser.privatebrowsing.autostart"
    ).value;
    let modePref = this._getAfaqCleanupModePreference();

    alwaysClearBox.checked =
      !privateBrowsing && this._isAfaqExitCleanupEnabled();
    alwaysClearBox.disabled = privateBrowsing || modePref.locked;
    settingsButton.disabled =
      privateBrowsing || !this._isAfaqExitCleanupEnabled();
    this._updateDeleteOnCloseBoxFromPrefs();
    void this._updateAfaqCleanupSummary();
  },

  _getAfaqSummaryItemL10nIds() {
    return {
      browsingHistoryAndDownloads: "history-clear-summary-item-history",
      cookiesAndStorage: "history-clear-summary-item-cookies",
      cache: "history-clear-summary-item-cache",
      formdata: "history-clear-summary-item-formdata",
      siteSettings: "history-clear-summary-item-site-settings",
    };
  },

  async _updateAfaqCleanupSummary() {
    let cleanupSummary = document.getElementById("afaqCleanupSummary");
    let exceptionsSummary = document.getElementById(
      "afaqCleanupExceptionsSummary"
    );
    if (!cleanupSummary || !exceptionsSummary) {
      return;
    }

    let requestId = ++this._afaqCleanupSummaryRequestId;
    if (
      this._isAfaqExitCleanupEnabled() &&
      this._hasAfaqSelectedExitCategories()
    ) {
      let itemL10nIds = [];
      for (let [item, pref] of Object.entries(this._getAfaqExitCleanupPrefs())) {
        if (pref.value) {
          itemL10nIds.push(this._getAfaqSummaryItemL10nIds()[item]);
        }
      }

      let itemLabels = await Promise.all(
        itemL10nIds.map(l10nId => document.l10n.formatValue(l10nId))
      );
      if (requestId != this._afaqCleanupSummaryRequestId) {
        return;
      }

      document.l10n.setAttributes(cleanupSummary, "history-clear-summary-exit", {
        items: itemLabels.join(", "),
      });
    } else {
      document.l10n.setAttributes(cleanupSummary, "history-clear-summary-keep");
    }

    document.l10n.setAttributes(
      exceptionsSummary,
      "history-clear-exceptions-summary",
      {
        count: AfaqCleanupService.getExceptionRules().length,
      }
    );
    await document.l10n.translateElements([cleanupSummary, exceptionsSummary]);
  },

  onAfaqAlwaysClear(event) {
    let historyMode = Preferences.get("privacy.history.custom");
    let modePref = this._getAfaqCleanupModePreference();

    if (event.target.checked) {
      this._ensureAfaqDefaultExitCategories();
      modePref.value = "exit";
  ...
  showAfaqCleanupExceptions() {
    let params = {
      blockVisible: true,
      sessionVisible: false,
      allowVisible: true,
      prefilledHost: "",
      permissionType: "afaq-cleanup",
      blockL10nId: "permissions-afaq-cleanup-always-clear",
      allowL10nId: "permissions-afaq-cleanup-preserve",
    };
    gSubDialog.open(
      "chrome://browser/content/preferences/dialogs/permissions.xhtml",
      {
        closingCallback() {
          void gPrivacyPane._updateAfaqCleanupSummary();
        },
      },
      params
    );
  ...
  syncSanitizationPrefsWithDeleteOnClose() {
    ...
    deleteOnCloseBox.addEventListener("command", () => {
      ...
      prefs.cookiesAndStorage.value = checked;
      prefs.cache.value = checked;

      if (checked) {
        modePref.value = "exit";
      } else if (!this._hasAfaqSelectedExitCategories()) {
        modePref.value = "keep";
      }
    ...
    prefs.cookiesAndStorage.on(
      "change",
      this._syncAfaqCleanupControls.bind(this)
    );
    prefs.cache.on("change", this._syncAfaqCleanupControls.bind(this));
    modePref.on("change", this._syncAfaqCleanupControls.bind(this));
  },
