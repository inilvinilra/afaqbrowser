...
nsWindowsShellService::CheckAllProgIDsExist(bool* aResult) {
  *aResult = false;
  ...
  if (widget::WinUtils::HasPackageIdentity()) {
    ...
    *aResult =
        CheckProgIDExists(FormatProgID(L"AfaqURL", aumid.get()).get()) &&
        CheckProgIDExists(FormatProgID(L"AfaqHTML", aumid.get()).get()) &&
        CheckProgIDExists(FormatProgID(L"AfaqPDF", aumid.get()).get());
  }
