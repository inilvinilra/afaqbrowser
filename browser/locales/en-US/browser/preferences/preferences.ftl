...
home-mode-choice-default-fx =
    .label = Null Browser Start Page (Default)
...
home-prefs-content-header2 = Null Browser Start Page Content
home-prefs-content-description2 = Choose what content you want on your Null Browser start page.
...
sync-mobile-promo = Download Null Browser for <img data-l10n-name="android-icon"/> <a data-l10n-name="android-link">Android</a> or <img data-l10n-name="ios-icon"/> <a data-l10n-name="ios-link">iOS</a> to sync with your mobile device.
...
sync-engine-addons =
    .label = Add-ons
    .tooltiptext = Extensions and themes for Null Browser desktop
    .accesskey = A
...
afaq-security-header = Null Browser Security

afaq-security-description = Select how aggressively Null Browser reduces browser fingerprinting, network metadata, and active web features.
...
afaq-security-level-summary-standard = Standard keeps the privacy baseline on while preserving the broadest site compatibility.

afaq-security-level-summary-hardened = Hardened is the recommended default. It enables stronger fingerprinting resistance, stricter network privacy, and disables WebRTC by default for a more deterministic privacy posture.
...
afaq-security-network-description = These controls keep Null Browser closer to a hardened desktop privacy profile while still allowing compatibility tuning when a site needs it.
...
afaq-security-advanced-description = These controls expose the browser-facing behaviors privacy-focused users usually tune manually in hardened privacy browsers.
...
afaq-security-webrtc-note = Protected keeps calls and conferencing available while hiding host candidates and local addresses. Disabled removes the WebRTC surface entirely, but many conferencing and calling sites will stop working. Some leak-test sites may still show incomplete or contradictory results when protected mode is active.
...
afaq-security-compatibility-description = Compatibility profiles target the most common daily-web breakage points without turning the browser into a generic low-privacy browser clone.
...
afaq-security-diagnostics-description = Null Browser diagnostics reflect the browser's own active privacy posture. When external leak-test sites disagree, prefer these values and corroborate them with more than one network test.
