...
Section "-InstallStartCleanup"
  ...
  ${InitHashAppModelId} "$INSTDIR" "Software\Afaq\${AppName}\TaskBarIDs"

  ; Clean up old maintenance service logs
  ${CleanMaintenanceServiceLogs} "Afaq\Browser"
...
Section "-Application" APP_IDX
  ...
  ${Else}
    ...
    ReadRegStr $0 HKLM "Software\afaq.org\Afaq" "CurrentVersion"
    ${If} "$0" != "${GREVersion}"
      WriteRegStr HKLM "Software\afaq.org\Afaq" "CurrentVersion" "${GREVersion}"
    ${EndIf}
  ${EndIf}
  ...
  ; On install always add the AfaqHTML-, AfaqPDF-, and AfaqURL- keys.
  ; An empty string is used for the 5th param because AfaqHTML- is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in AfaqURL- and
  ; AfaqHTML- to launch the desktop browser when it needs to.
  ${AddDisabledDDEHandlerValues} "AfaqHTML-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                 "${AppRegName} HTML Document" ""
  ${AddDisabledDDEHandlerValues} "AfaqPDF-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
                                 "${AppRegName} PDF Document" ""
  ${AddDisabledDDEHandlerValues} "AfaqURL-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                 "${AppRegName} URL" "true"
...
!ifdef MOZ_DEFAULT_BROWSER_AGENT
  ...
  WriteRegDWORD HKCU "Software\Afaq\${AppName}\Installer\$AppUserModelID" \
...
Function SendPing
  ...
  ClearErrors
  WriteRegStr HKLM "Software\Afaq" "${BrandShortName}InstallerTest" \
                   "Write Test"
  ${If} ${Errors}
...
Function preComponents
  ...
  ClearErrors
  WriteRegStr HKLM "Software\Afaq" \
...
!endif
  ...
  SetShellVarContext all
  ${GetFirstInstallPath} "Software\Afaq\${BrandFullNameInternal}" $0
  ${If} "$0" == "false"
    SetShellVarContext current
    ${GetFirstInstallPath} "Software\Afaq\${BrandFullNameInternal}" $0
    ${If} "$0" == "false"
