...
!define MaintUninstallKey \
 "Software\Microsoft\Windows\CurrentVersion\Uninstall\AfaqMaintenanceService"
...
Function un.UninstallServiceIfNotUsed
  ...
  StrCpy $0 0
  ${Do}
    EnumRegKey $1 HKLM "Software\Afaq\MaintenanceService" $0
    ${If} "$1" == ""
      ${ExitDo}
    ${EndIf}
    IntOp $0 $0 + 1
...
Section "Uninstall"
  ...
  ${un.RegCleanMain} "Software\Afaq"
  ${un.RegCleanPrefs} "Software\Afaq\${AppName}"
  ...
  ${un.CleanMaintenanceServiceLogs} "Afaq\Browser"

  ; Remove any app model id's stored in the registry for this install path
  DeleteRegValue HKCU "Software\Afaq\${AppName}\TaskBarIDs" "$INSTDIR"
  DeleteRegValue HKLM "Software\Afaq\${AppName}\TaskBarIDs" "$INSTDIR"

  ClearErrors
  WriteRegStr HKLM "Software\Afaq" "${BrandShortName}InstallerTest" "Write Test"
  ${If} ${Errors}
    StrCpy $RegHive "HKCU"
  ${Else}
    SetShellVarContext all  ; Set SHCTX to HKLM
    DeleteRegValue HKLM "Software\Afaq" "${BrandShortName}InstallerTest"
    StrCpy $RegHive "HKLM"
    ${un.RegCleanMain} "Software\Afaq"
    ${un.RegCleanUninstall}
    ${un.DeleteShortcuts}
  ...
  ${un.RegCleanAppHandler} "AfaqHTML-$AppUserModelID"
  ${un.RegCleanAppHandler} "AfaqPDF-$AppUserModelID"
  ${un.RegCleanAppHandler} "AfaqURL-$AppUserModelID"
  ${un.RegCleanProtocolHandler} "http"
  ${un.RegCleanProtocolHandler} "https"
  ${un.RegCleanProtocolHandler} "mailto"
  ${un.RegCleanFileHandler}  ".htm"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".html"  "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".shtml" "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".xht"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".xhtml" "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".oga"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".ogg"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".ogv"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".webm"  "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".svg"   "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".webp"  "AfaqHTML-$AppUserModelID"
  ${un.RegCleanFileHandler}  ".avif"  "AfaqHTML-$AppUserModelID"

  ${un.RegCleanFileHandler}  ".pdf"   "AfaqPDF-$AppUserModelID"

  SetShellVarContext all  ; Set SHCTX to HKLM
  ${un.GetSecondInstallPath} "Software\Afaq" $R9
  ${If} $R9 == "false"
    SetShellVarContext current  ; Set SHCTX to HKCU
    ${un.GetSecondInstallPath} "Software\Afaq" $R9
  ${EndIf}
  ...
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Afaq-${AppName}-$AppUserModelID"
  ...
  ; bare AfaqPDF.
  ReadRegStr $0 HKLM "Software\Classes\AfaqHTML\DefaultIcon" ""
  StrCpy $0 $0 -2
  ${If} $0 == "$INSTDIR\${FileMainEXE}"
    DeleteRegKey HKLM "Software\Classes\AfaqHTML"
    DeleteRegKey HKLM "Software\Classes\AfaqURL"
  ...
  ${EndIf}
  ReadRegStr $0 HKCU "Software\Classes\AfaqHTML\DefaultIcon" ""
  StrCpy $0 $0 -2
  ${If} $0 == "$INSTDIR\${FileMainEXE}"
    DeleteRegKey HKCU "Software\Classes\AfaqHTML"
    DeleteRegKey HKCU "Software\Classes\AfaqURL"
    ${StrFilter} "${FileMainEXE}" "+" "" "" $R9
...
!endif
  ...
  WriteRegStr HKCU "Software\Afaq\Browser" "Uninstalled-${UpdateChannel}" "True"
...
!ifdef MOZ_BITS_DOWNLOAD
  BitsUtils::CancelBitsJobsByName "AfaqUpdate $AppUserModelID"
  Pop $0
!endif
...
Function un.onInit
  ...
  ${un.InitHashAppModelId} "$INSTDIR" "Software\Afaq\${AppName}\TaskBarIDs"
