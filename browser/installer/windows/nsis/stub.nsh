...
Function ShouldPromptForProfileCleanup
  ...
  ${If} $R0 == ""
    ...
    ${Do}
      ...
      ReadINIStr $1 "$APPDATA\Afaq\Browser\profiles.ini" "Profile$0" "Path"
      ${If} ${Errors}
      ...
      ClearErrors
      ReadINIStr $1 "$APPDATA\Afaq\Browser\profiles.ini" "Profile$0" "Default"
      ${IfNot} ${Errors}
      ${AndIf} $1 == "1"
        ; We've found the default profile
        ReadINIStr $1 "$APPDATA\Afaq\Browser\profiles.ini" "Profile$0" "Path"
        ReadINIStr $2 "$APPDATA\Afaq\Browser\profiles.ini" "Profile$0" "IsRelative"
        ${If} $2 == "1"
          StrCpy $R0 "$APPDATA\Afaq\Browser\$1"
        ${Else}
          StrCpy $R0 "$1"
        ${EndIf}
        ${Break}
  ...
  ; installation present if HKCR\AfaqURL* exists and points to a real path.
  StrCpy $0 0
  StrCpy $R9 ""
  ${Do}
    ClearErrors
  ...
  ${GetSingleInstallPath} "Software\Afaq\${BrandFullNameInternal}" $0
  ${If} $0 == "false"
    SetShellVarContext current
    ${GetSingleInstallPath} "Software\Afaq\${BrandFullNameInternal}" $0
    ${If} $0 == "false"
