...
!macro PostUpdate
  ...
  ; install location in the Software\Afaq key and uninstall registry entries
  ...
  ${If} ${FileExists} "$INSTDIR\postSigningData"
    ...
    Delete /REBOOTOK "$0\Afaq\Browser\postSigningData"
  ${Else}
    ${If} ${FileExists} "$0\Afaq\Browser\postSigningData"
      Rename "$0\Afaq\Browser\postSigningData" "$INSTDIR\postSigningData"
    ${EndIf}
  ${EndIf}
...
!ifdef MOZ_MAINTENANCE_SERVICE
  ...
  ${AndIf} $RegHive == "HKLM"
    ...
    ${OrIf} ${IsNativeARM64}
      SetRegView 64
    ${EndIf}
    ReadRegDWORD $5 HKLM "Software\Afaq\MaintenanceService" "Attempted"
    ClearErrors
    ${If} ${RunningX64}
    ${OrIf} ${IsNativeARM64}
      SetRegView lastused
...
${If} $RegHive == "HKCU"
  ClearErrors
  ReadRegDWORD $0 HKCU "Software\Afaq\${AppName}\Installer\$AppUserModelID" \
...
!macro SetHandlers
  ...
  ; Associate the file handlers with AfaqHTML, if they aren't already.
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "AfaqHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "AfaqHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "AfaqHTML"
    WriteRegStr SHCTX "$0\.html"  "" "AfaqHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "AfaqHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "AfaqHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "AfaqHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "AfaqHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "AfaqHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "AfaqHTML$5"
  ${EndIf}
  ...
  ${AddAssociationIfNoneExist} ".oga" "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".ogg" "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".ogv" "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".webm" "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".svg" "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".webp"  "AfaqHTML$5"
  ${AddAssociationIfNoneExist} ".avif" "AfaqHTML$5"

  ${AddAssociationIfNoneExist} ".pdf" "AfaqPDF$5"

  ; An empty string is used for the 5th param because AfaqHTML- is not a
  ; protocol handler.  Ditto for AfaqPDF-.
  ${AddDisabledDDEHandlerValues} "AfaqHTML$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "AfaqPDF$5" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
                                 "${AppRegName} PDF Document" ""

  ${AddDisabledDDEHandlerValues} "AfaqURL$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" "${AppRegName} URL" \
                                 "true"
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".svg"   "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".webp"  "AfaqHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".avif"  "AfaqHTML$2"

  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".pdf"   "AfaqPDF$2"
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "AfaqURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "AfaqURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "mailto" "AfaqURL$2"
...
!ifdef HAVE_64BIT_BUILD
  ...
  ClearErrors
  ReadRegDWORD $2 HKLM "Software\Afaq\${AppName}\32to64DidMigrate" "$1"
  ...
  ${AndIf} $2 != 1
    ClearErrors
    WriteRegDWORD HKLM "Software\Afaq\${AppName}\32to64DidMigrate" "$1" 1
    ${If} ${Errors}
      ; There was an error writing to HKLM so just write it to HKCU
      WriteRegDWORD HKCU "Software\Afaq\${AppName}\32to64DidMigrate" "$1" 1
    ${Else}
      ; This will delete the value from HKCU if it exists
      DeleteRegValue HKCU "Software\Afaq\${AppName}\32to64DidMigrate" "$1"
    ${EndIf}
  ${EndIf}

  ClearErrors
  ReadRegDWORD $2 HKCU "Software\Afaq\${AppName}\32to64DidMigrate" "$1"
  ; If there were no errors then the system was updated from Firefox 32 bit to
  ; Firefox 64 bit and if the value is already 1 then the registry value has
...
!else
  ...
  ${OrIf} ${IsNativeARM64}
    ...
    ${If} ${Errors}
      ClearErrors
      WriteRegDWORD HKLM "Software\Afaq\${AppName}\32to64DidMigrate" "$1" 0
      ; If there were errors write the value in HKCU.
      ${If} ${Errors}
        WriteRegDWORD HKCU "Software\Afaq\${AppName}\32to64DidMigrate" "$1" 0
      ${EndIf}
    ${EndIf}
  ${EndIf}
...
; The IconHandler reference for AfaqHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with Firefox (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ; Find the correct key to update, either AfaqHTML or AfaqHTML-[PathHash]
  StrCpy $3 "AfaqHTML-$AppUserModelID"
  ClearErrors
...
; Add Software\Afaq\ registry entries (uses SHCTX).
...
!macro SetAppKeys
  ...
  StrCpy $0 "Software\Afaq\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Main"
  ${WriteRegStr2} $RegHive "$0" "Install Directory" "$8" 0
  ${WriteRegStr2} $RegHive "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\Afaq\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Uninstall"
  ${WriteRegStr2} $RegHive "$0" "Description" "${BrandFullNameInternal} ${AppVersion}$3 (${ARCH} ${AB_CD})" 0

  StrCpy $0 "Software\Afaq\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})"
  ...
  StrCpy $0 "Software\Afaq\${BrandFullNameInternal} ${AppVersion}$3\bin"
  ${WriteRegStr2} $RegHive "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\Afaq\${BrandFullNameInternal} ${AppVersion}$3\extensions"
  ${WriteRegStr2} $RegHive "$0" "Components" "$8\components" 0
  ${WriteRegStr2} $RegHive "$0" "Plugins" "$8\plugins" 0

  StrCpy $0 "Software\Afaq\${BrandFullNameInternal} ${AppVersion}$3"
  ...
  StrCpy $0 "Software\Afaq\${BrandFullNameInternal}$3"
...
!macro SetUninstallKeys
  ...
  ${If} $2 == ""
    ...
    ${WriteRegStr2} $1 "$0" "Publisher" "Afaq" 0
...
; to AfaqHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove AfaqHTML as the default
    ; value from both HKCU and HKLM if it set to AfaqHTML.
    ${If} "$0" == "AfaqHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "AfaqHTML"
    ; Since HKCU is set to AfaqHTML remove AfaqHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "AfaqHTML-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because AfaqHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "AfaqHTML-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} HTML Document" ""
  ${Else}
    ${IsHandlerForInstallDir} "AfaqHTML" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "AfaqHTML" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} HTML Document" ""
    ${EndIf}
  ${EndIf}

  ; AfaqPDF-* was added after AfaqHTML and AfaqURL, so we've never
  ; supported bare "AfaqPDF".  But we won't have it from the installer, so we
  ...
  ${IsHandlerForInstallDir} "AfaqURL-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "AfaqURL-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} URL" "true"
  ${Else}
    ${IsHandlerForInstallDir} "AfaqURL" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "AfaqURL" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} URL" "true"
    ${EndIf}
  ${EndIf}
...
!macro MigrateTaskBarShortcut SHOULD_PIN
  ...
  ${If} ${FileExists} "$0"
    ...
    ${If} ${Errors}
      ...
      WriteRegDWORD HKCU \
        "Software\Afaq\${AppName}\Installer\$AppUserModelID" \
