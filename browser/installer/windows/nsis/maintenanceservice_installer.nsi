...
; this installed.
!define FallbackKey \
  "SOFTWARE\Afaq\MaintenanceService\3932ecacee736d366d6436db0f55bce4"

!define CompanyName "Afaq"
...
!define MaintFullName "Afaq Maintenance Service"
!ifdef BrandFullName
!undef BrandFullName
!endif
...
InstallDirRegKey HKLM "Software\Afaq\MaintenanceService" ""
...
Section "MaintenanceService"
  ...
  WriteRegStr HKLM "${MaintUninstallKey}" "Publisher" "Afaq"
  ...
  WriteRegDWORD HKLM "Software\Afaq\MaintenanceService" "Attempted" 1
  WriteRegDWORD HKLM "Software\Afaq\MaintenanceService" "Installed" 1
  DeleteRegValue HKLM "Software\Afaq\MaintenanceService" "FFPrefetchDisabled"

  ; Included here for debug purposes only.  
  ; These keys are used to bypass the installation dir is a valid installation
  ; check from the service so that tests can be run.
  ; WriteRegStr HKLM "${FallbackKey}\0" "name" "Afaq"
...
Section "Uninstall"
  ...
  Push "$APPDATA\Afaq\logs\maintenanceservice.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-1.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-2.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-3.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-4.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-5.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-6.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-7.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-8.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-9.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-10.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-install.log"
  Call un.RenameDelete
  Push "$APPDATA\Afaq\logs\maintenanceservice-uninstall.log"
  Call un.RenameDelete
  RMDir /REBOOTOK "$APPDATA\Afaq\logs"
  RMDir /REBOOTOK "$APPDATA\Afaq"
  ...
  ${EndIf}
  DeleteRegKey HKLM "${MaintUninstallKey}"
  DeleteRegValue HKLM "Software\Afaq\MaintenanceService" "Installed"
  DeleteRegValue HKLM "Software\Afaq\MaintenanceService" "FFPrefetchDisabled"
