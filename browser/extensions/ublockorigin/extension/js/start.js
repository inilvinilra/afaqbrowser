...
const initializeTabs = async ( ) => {
    const manifest = self.__afaqResolveManifest instanceof Function
        ? self.__afaqResolveManifest()
        : undefined;
...
const onUserSettingsReady = fetched => {
    const shouldPersistDarkTheme =
        typeof fetched.uiTheme !== 'string' ||
        fetched.uiTheme === '' ||
        fetched.uiTheme === 'auto';
    if ( shouldPersistDarkTheme ) {
        fetched.uiTheme = 'dark';
    }
...
{
    const runtimeAPI = self.__afaqResolveRuntimeAPI instanceof Function
        ? self.__afaqResolveRuntimeAPI()
        : undefined;
    if ( runtimeAPI?.onUpdateAvailable?.addListener instanceof Function ) {
