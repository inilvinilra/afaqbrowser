...
var vAPI = self.vAPI = self.vAPI || { uBO: true };
...
const resolveBackgroundManifest = ( ) => {
    if ( self.__afaqResolveManifest instanceof Function ) {
        return self.__afaqResolveManifest();
    }
    return {
        name: 'uBlock Origin',
        version: '0.0.0',
...
const resolveBackgroundRuntimeAPI = ( ) => {
    if ( self.__afaqResolveRuntimeAPI instanceof Function ) {
        return self.__afaqResolveRuntimeAPI();
    }
};

const resolveBackgroundStorageAPI = ( ) => {
    const candidate = self.__afaqResolveWebextAPI instanceof Function
        ? self.__afaqResolveWebextAPI()
        : undefined;
    return candidate?.storage instanceof Object
        ? candidate.storage
        : undefined;
};

const resolveBackgroundWebRequestAPI = ( ) => {
    const candidate = self.__afaqResolveWebextAPI instanceof Function
        ? self.__afaqResolveWebextAPI()
        : undefined;
    return candidate?.webRequest instanceof Object
        ? candidate.webRequest
        : undefined;
};

const resolveBackgroundStorageLocal = ( ) => {
    return resolveBackgroundStorageAPI()?.local;
};

const manifest = resolveBackgroundManifest();
const runtimeAPI = resolveBackgroundRuntimeAPI();
const tabsAPI = webext.tabs;
const windowsAPI = webext.windows;
const webNavigationAPI = webext.webNavigation;
const webRequestAPI = resolveBackgroundWebRequestAPI();

vAPI.cantWebsocket =
    webRequestAPI?.ResourceType instanceof Object === false  ||
    webRequestAPI?.ResourceType?.WEBSOCKET !== 'websocket';
...
vAPI.app = {
    ...
    restart: function() {
        const runtimeAPI = resolveBackgroundRuntimeAPI();
        if ( runtimeAPI?.reload instanceof Function ) {
            runtimeAPI.reload();
        }
    },
};
...
vAPI.sessionStorage = resolveBackgroundStorageAPI()?.session || {
    get() {
...
vAPI.storage = {
    get(key, ...args) {
        const storageLocal = resolveBackgroundStorageLocal();
        if ( storageLocal?.get instanceof Function === false ) {
            return Promise.resolve();
        }
        return storageLocal.get(key, ...args).catch(reason => {
            console.log(reason);
        });
    },
    set(...args) {
        const storageLocal = resolveBackgroundStorageLocal();
        if ( storageLocal?.set instanceof Function === false ) {
            return Promise.resolve();
        }
    ...
    remove(...args) {
        const storageLocal = resolveBackgroundStorageLocal();
        if ( storageLocal?.remove instanceof Function === false ) {
            return Promise.resolve();
        }
    ...
    clear(...args) {
        const storageLocal = resolveBackgroundStorageLocal();
        if ( storageLocal?.clear instanceof Function === false ) {
            return Promise.resolve();
        }
    ...
    QUOTA_BYTES: resolveBackgroundStorageLocal()?.QUOTA_BYTES || 0,
};

// Not all platforms support getBytesInUse
if ( resolveBackgroundStorageLocal()?.getBytesInUse instanceof Function ) {
    vAPI.storage.getBytesInUse = function(...args) {
        return resolveBackgroundStorageLocal().getBytesInUse(...args).catch(reason => {
            console.log(reason);
        });
    };
}
...
vAPI.Tabs = class {
    constructor() {
        webNavigationAPI?.onCreatedNavigationTarget?.addListener(details => {
        ...
        tabsAPI?.onActivated?.addListener(details => {
            this.onActivated(details);
        });
        if ( windowsAPI instanceof Object ) {
            windowsAPI.onFocusChanged?.addListener(windowId => {
                this.onFocusChangedHandler(windowId);
            });
        }
        tabsAPI?.onRemoved?.addListener((tabId, details) => {
            if ( vAPI.net && vAPI.net.hasUnprocessedRequest(tabId) ) {
                vAPI.net.removeUnprocessedRequest(tabId);
            }
    ...
    async create(url, details) {
        ...
        const subWrapper = async ( ) => {
            ...
            if ( !details.tabId ) {
                ...
                let tab = null;
                try {
                    tab = await tabsAPI?.create?.(updateDetails);
                } catch {
                }
            ...
            if ( tab === null ) {
                let createdTab = null;
                try {
                    createdTab = await tabsAPI?.create?.(updateDetails);
                } catch {
                }
                if ( createdTab instanceof Object ) {
                    focusWindow(createdTab);
                }
            } else if ( details.index !== undefined && tabsAPI?.move instanceof Function ) {
                tabsAPI.move(tab.id, { index: details.index });
            }
        };
...
vAPI.browserAction = vAPI.browserAction || {};

if ( webext.browserAction instanceof Object ) {
    vAPI.browserAction.setTitle = async function() {
        try {
...

{
    ...
    const titleTemplate = `${manifest.browser_action?.default_title || manifest.action?.default_title || manifest.name} ({badge})`;
    const icons = [
        { path: {
...
if ( webext.browserAction?.onClicked?.addListener instanceof Function ) {
    webext.browserAction.onClicked.addListener(function(tab) {
        vAPI.tabs.open({
            select: true,
...
vAPI.messaging = {
    ...
    setup: function(defaultHandler) {
        ...
        runtimeAPI?.onConnect?.addListener(
...

{
    ...
    if ( webext.webRequest?.onBeforeRequest?.addListener instanceof Function ) {
...
vAPI.Net = class {
    constructor() {
        ...
        {
            const wrrt = webRequestAPI?.ResourceType || {};
    ...
    addListener(which, clientListener, filters, options) {
        ...
        if ( webRequestAPI?.[which]?.addListener instanceof Function === false ) {
            return;
        }
        webRequestAPI[which].addListener(
            actualListener,
            actualFilters,
            options
        );
    ...
    removeListener(which, clientListener) {
        ...
        webRequestAPI?.[which]?.removeListener?.(actualListener);
    }
    ...
    handlerBehaviorChanged() {
        webRequestAPI?.handlerBehaviorChanged?.();
    }
...
vAPI.cloud = (( ) => {
    ...
    const syncStorage = webext.storage?.sync;
    if ( syncStorage instanceof Object === false ) { return; }
    ...
    const MAX_ITEMS =
        syncStorage.MAX_ITEMS || 512;
    const QUOTA_BYTES =
        syncStorage.QUOTA_BYTES || 102400;
    const QUOTA_BYTES_PER_ITEM =
        syncStorage.QUOTA_BYTES_PER_ITEM || 8192;
    ...
    const getCoarseChunkCount = async function(datakey) {
        ...
        let bin;
        try {
            bin = await syncStorage.get(keys);
        } catch (reason) {
    ...
    const pull = async function(details) {
        ...
        let bin;
        try {
            bin = await syncStorage.get(chunkKeys);
        } catch (reason) {
            return String(reason);
        }
    ...
    const used = async function(datakey) {
        if ( syncStorage.getBytesInUse instanceof Function === false ) {
            return;
        }
        ...
        try {
            results = await Promise.all([
                syncStorage.getBytesInUse(keys),
                syncStorage.getBytesInUse(null),
            ]);
        } catch {
        }
