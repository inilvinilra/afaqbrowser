...
const resolveCommonExtensionURL = (path = '') => {
    if ( self.__afaqResolveURL instanceof Function ) {
        return self.__afaqResolveURL(path);
    }
    return path;
};
...
(( ) => {
    ...
    const manifest = resolveCommonManifest();
    ...
    const extensionOrigin = resolveCommonExtensionURL('');
...
vAPI.getURL = path => resolveCommonExtensionURL(path);
