...
const webRequestAPI = webext.webRequest;
...
const bodyFilterer = (( ) => {
    ...
    return class Session extends µb.FilteringContext {
        ...
        static doFilter(requestId, fctxt, jobs) {
            ...
            if ( webRequestAPI?.filterResponseData instanceof Function === false ) {
                return;
            }
...
const requestHeadersManager = {
    ...
    start() {
        ...
        webRequestAPI?.onSendHeaders?.addListener(details => {
            this.requests.set(details.requestId, details.requestHeaders);
        }, {
            urls: [ 'http://*/*', 'https://*/*' ]
        }, extraInfoSpec);
        webRequestAPI?.onBeforeRedirect?.addListener(details => {
