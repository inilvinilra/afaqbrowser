...
async function fetchBootstrapDetails() {
    let details = await assets.fetchText(
    ...
    if (
        typeof µb.assetsBootstrapFallbackPath === 'string' &&
        µb.assetsBootstrapFallbackPath !== '' &&
        µb.assetsBootstrapFallbackPath !== µb.assetsJsonPath
    ) {
        details = await assets.fetchText(µb.assetsBootstrapFallbackPath);
    }
    return details;
}
...
function getAssetSourceRegistry() {
    if ( assetSourceRegistryPromise === undefined ) {
        assetSourceRegistryPromise = cacheStorage.get(
            ...
            return fetchBootstrapDetails().then(details => {
...
async function updateNext() {
    ...
    if ( assetKey !== 'assets.json' || µb.hiddenSettings.debugAssetsJson !== true ) {
        ...
        result = await fetchBootstrapDetails();
