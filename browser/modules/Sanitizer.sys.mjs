...
export var Sanitizer = {
  ...
  async sanitize(itemsToClear = null, options = {}) {
    ...
    let trigger = options.trigger || (progress.isShutdown ? "exit" : "manual");
    ...
    try {
      ...
      try {
        await lazy.AfaqCleanupService.recordSanitization({
          trigger,
          itemsToClear,
          status,
          error,
        ...
        console.error("Failed to record Afaq cleanup audit", auditError);
      }
    }
    return progress;
  },
...
async function sanitizeOnShutdown(progress) {
  ...
  if (Sanitizer.shouldSanitizeOnShutdown) {
    ...
    await Sanitizer.sanitize(itemsToClear, { progress, trigger: "exit" });
  ...
  let afaqPrincipals = await gPrincipalsCollector.getAllPrincipals(progress);
  let afaqSplit =
    lazy.AfaqCleanupService.splitPrincipalsByExceptionRules(afaqPrincipals);
  progress.afaqCleanupForcedPrincipals = afaqSplit.forced.length;

  if (afaqSplit.forced.length) {
    progress.advancement = "afaq-cleanup-exceptions";
    await Promise.all(
      afaqSplit.forced.map(principal =>
...
async function maybeSanitizeSessionPrincipals(progress, principals, flags) {
  ...
  let split = lazy.AfaqCleanupService.splitPrincipalsByExceptionRules(principals);
  progress.afaqPreservedPrincipals = split.preserved.length;
  principals = split.selected;
