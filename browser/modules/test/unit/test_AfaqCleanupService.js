...
add_task(async function test_exception_rule_precedence() {
  ...
  let rootMatched = AfaqCleanupService.getExceptionRuleForPrincipal(
    Services.scriptSecurityManager.createContentPrincipalFromOrigin(
  ...
  Assert.equal(
    rootMatched,
    AfaqCleanupService.RULE_PRESERVE,
    "Root preserve must apply to child subdomains"
  );
  ...
  Assert.equal(
    AfaqCleanupService.getExceptionRuleForPrincipal(unrelatedPrincipal),
    null,
    "Unrelated domains must not inherit cleanup exception rules"
  );
  ...
  Assert.equal(split.forced.length, 2);
  Assert.equal(split.selected.length, 3);
});
