 ...
 *
 * @param   _KEY
 *          The registry subkey (typically this will be Software\Afaq).
 * @return  _RESULT
 ...
 *
 * @param   _KEY
 *          The registry subkey (typically this will be Software\Afaq\App Name).
 * @return  _RESULT
 ...
 *
 * @param   _KEY
 *          The registry subkey (typically Software\Afaq\App Name).
 * @return  _RESULT
 ...
 * @param   _KEY ($R1)
 *          The registry subkey
 *          (typically this will be Software\Afaq\${AppName}).
 */
!macro RegCleanPrefs
    ...
    Function ${_MOZFUNC_UN}CleanMaintenanceServiceLogs
      ...
      ${AndIf} $R6 != "" ; We have the install dir path
        ...
        ${If} $R1 != ""
          ...
          Delete "$PROGRAMFILES32\Afaq Maintenance Service\UpdateLogs\$R1.*"
          ...
          RmDir "$PROGRAMFILES32\Afaq Maintenance Service\UpdateLogs"
        ${EndIf}
      ${EndIf}
...
!ifndef NO_INSTDIR_FROM_REG
      ...
      ${OrIf} ${IsNativeARM64}
        SetRegView 64
        ${GetSingleInstallPath} "Software\Afaq\${BrandFullNameInternal}" $R9
        SetRegView lastused
      ${EndIf}
 ...
 *          be stored. ex: "Software\Afaq\Firefox\TaskBarIDs"
...
!macro FindInstallSpecificProfileMaybeUn _MOZFUNC_UN
  ...
  ${If} ${FileExists} "$APPDATA\Afaq\Afaq\installs.ini"
    ClearErrors
    ReadINIStr $1 "$APPDATA\Afaq\Afaq\installs.ini" "$AppUserModelID" "Default"
    ${IfNot} ${Errors}
      ${${_MOZFUNC_UN}GetLongPath} "$APPDATA\Afaq\Afaq\$1" $2
      ${If} ${FileExists} $2
        StrCpy $R0 $2
      ${Else}
